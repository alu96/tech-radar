  ...
  function ($log, radarService) {
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        var width = attrs.width, height = attrs.height, padding = 30, diagramRadius = Math.min(attrs.width, attrs.height) / 2 - padding;
        ...
        var svgArcs = svg.append('g').attr('transform', 'translate(' + (width / 2 - padding) + ',' + (height / 2 - padding) + ')');
        var svgNodes = svg.append('g').attr('transform', 'translate(' + (width / 2 - padding) + ',' + (height / 2 - padding) + ')');
        /**
       ...
       */
        ...
        function applyRandomXY(arc, d) {
          ...
          inner = arc.startAngle + angularBuffer;
          outer = arc.endAngle - angularBuffer;
        ...
        var arcCategoryEnter = arcStatusEnter.selectAll('path').data(function (d) {
            return d.categories;
          }).enter().append('g').attr('class', 'slice');
        arcCategoryEnter.append('path').attr('fill', function (d, slice, ring) {
          return colorFiveGroupsOfSeven(7 * slice + ring + 3);
        }).attr('stroke', 'grey').attr('stroke-width', '1px').attr('stroke-opacity', '.25').datum(function (d, i, j) {
        ...
        var nodeCategoryEnter = nodeStatusEnter.selectAll('g').data(function (d) {
            return d.categories;
          }).enter().append('g').datum(function (category, categoryIndex) {
        ...
        function getTechLabelSubstring(label) {
          return label.length <= truncatedLabelLength ? label : label.substring(0, truncatedLabelLength - 1) + '...';
        }
        function drawTech() {
          technologies = nodeCategoryEnter.selectAll('g').data(function (d) {
            return d.technologies;
          });
          $log.info('Redrawing');
          var techEnter = technologies.enter().append('g').attr('class', 'tech-label').on('mouseover', function (d) {
              d.active = true;
              redrawTechCircles();
            }).on('mouseout', function (d) {
              d.active = false;
          ...
          techEnter.append('text').datum(function (d) {
          ...
          techEnter.append('circle').attr('r', defaultTechRadius).style('stroke', 'grey').style('fill', 'whitesmoke').attr('cx', function (d) {
            return d.x;
          }).attr('cy', function (d) {
            return d.y;
          });
        ...
        function redrawTechCircles() {
          scope.$apply();
          technologies.selectAll('text').transition().duration(150).tween('text', function (d) {
          ...
          technologies.selectAll('circle').transition().duration(500).attr('r', function (d) {
